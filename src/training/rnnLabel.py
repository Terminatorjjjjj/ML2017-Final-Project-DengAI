...
log = (str(city)+'_'+ str(weekConcat) + '_' + str(add)+ '_' + str(rnnLayer) + '_'
      + str(hidLayer)+'_'+str(weekPredAfter))
...
def reshape(train_feature, train_label):
    ...
    for i in range(0,trainRow):
        ...
        trainL[i] = train_label[i+((weekConcat-1)+weekPredAfter)] 
    return trainF,  trainL

def setPeakIdx():
    ## peak label
    peak = np.hstack((np.arange(20,33),np.arange(72,85)))
    peak = np.hstack((peak,np.arange(124,138)))
    peak = np.hstack((peak,np.arange(177,190)))
    peak = np.hstack((peak,np.arange(229,242)))
    peak = np.hstack((peak,np.arange(281,293)))
    peak = np.hstack((peak,np.arange(332,345)))
    peak = np.hstack((peak,np.arange(384,397)))
    peak = np.hstack((peak,np.arange(433,450)))
    peak = np.hstack((peak,np.arange(490,502)))
    peak = np.hstack((peak,np.arange(539,553)))
    peak = np.hstack((peak,np.arange(592,605)))
    peak = np.hstack((peak,np.arange(644,657)))
    peak = np.hstack((peak,np.arange(696,709)))
    peak = np.hstack((peak,np.arange(748,762)))
    peak = np.hstack((peak,np.arange(801,814)))
    peak = np.hstack((peak,np.arange(853,865)))
    peak = np.hstack((peak,np.arange(904,917)))
...
ck = ModelCheckpoint('savedModel/'+log+'-{epoch:02d}-{val_loss:.2f}.hdf5', 
                     verbose=1,
                     save_best_only=True,
...
if peakFlag == True and city == 'sj':
    row = train_feature.shape[0]
    peak = setPeakIdx()
    peakExcept = []
    for i in range(0,row):
        if np.size(np.where(i == peak)[0]) == 0:
            peakExcept.append(i)
    peakExcept = np.asarray(peakExcept)
    
    x_train = train_feature[peakExcept]
    y_train = train_label[peakExcept]
    x_val = train_feature[peak]
    y_val = train_label[peak]    
else:    
    (x_train,y_train),(x_val,y_val) = split_data(train_feature, train_label, 0.1)
    
#plot_model(model,to_file='modelrnn.png')
